...
| `MERIDIAN_KEEPER_RPC_TIMEOUT_MS`         | No           | `10000`                            | Timeout for keeper RPC calls, in milliseconds. Fully governs submission calls; discovery's `get_adapter`/`get_protocol` reads are additionally capped at a hardcoded 10s ceiling shared with the rest of `stellar-sdk-helpers`, so values above `10000` only extend the submission side.                                 |
...
| `MERIDIAN_MIGRATION_MAX_SLIPPAGE_BPS`    | No           | `100`                              | `max_slippage_bps` passed to every `migrate_adapter` call. The config loader rejects `10000` (unlimited slippage).                                                                                                                                                                                                       |
...
| `MERIDIAN_BLEND_ADAPTER_ID`              | No           | `""`                               | Candidate Blend adapter contract address the migration keeper may migrate the vault to. Empty excludes Blend from consideration.                                                                                                                                                                                         |
| `MERIDIAN_DEFINDEX_ADAPTER_ID`           | No           | `""`                               | Candidate DeFindex adapter contract address. Falls back to `CONTRACT_ADDRESSES.testnet.defindex.adapter` when unset.                                                                                                                                                                                                     |

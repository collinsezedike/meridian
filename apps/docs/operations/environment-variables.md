...
| `REDIS_URL`         | No       | `""`                               | Redis connection URL (`rediss://…`) used as the `@fastify/rate-limit` store in `apps/api-local` via ioredis. When unset, rate limits use an in-memory store (single process only); set in production for distributed rate limiting across instances. |

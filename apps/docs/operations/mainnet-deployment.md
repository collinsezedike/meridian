...
- **`deploy-testnet.sh` hardcodes `NETWORK="testnet"` (line 12) and is not mainnet-safe to point at mainnet by an env var alone.** It also silently defaults `ADMIN` to `DEPLOYER`'s own address with just a warning, which is exactly the kind of convenience mainnet cannot tolerate. A hardened `deploy-mainnet.sh` (hard-required `ADMIN`/`ADMIN_KEY`, a typed confirmation before submitting, and an allow-list for `USDC_ID`/`BLEND_POOL_ID`) is tracked separately as #717 and is not yet written. Until it lands, run the same command sequence by hand with `--network mainnet` substituted throughout, verifying each step's output before moving to the next. See "Deployment sequence" below, which lists that sequence explicitly for exactly this reason.
...
- [ ] #717 (`deploy-mainnet.sh`) landed, or the manual command sequence in "Deployment sequence" rehearsed at least once against testnet by whoever will run it for real

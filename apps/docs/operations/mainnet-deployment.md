...
- **`deploy-testnet.sh` hardcodes `NETWORK="testnet"` (line 12) and is not mainnet-safe to point at mainnet by an env var alone.** There is no `NETWORK=mainnet bash scripts/deploy-testnet.sh` shortcut today. Either adapt the script (parameterize `NETWORK`, drop the Friendbot-funding assumption) before running it, or run the same command sequence by hand with `--network mainnet` substituted throughout, verifying each step's output before moving to the next. See "Deployment sequence" below, which lists that sequence explicitly for exactly this reason.
...
Mirrors "Standing up a fresh environment" in the testnet doc exactly, with `--network mainnet` (or `NETWORK="mainnet"` if you've adapted the script per the prerequisite above) substituted throughout, and real funding instead of Friendbot:

1. Reserve the vault's contract address via `stellar contract id wasm --network mainnet --source-account $ADMIN_ADDR --salt <salt>`. Choose and record the salt deliberately: it is part of what makes the address reproducible later if you ever need to re-verify it.
2. Deploy the Blend adapter with that reserved vault address and USDC as constructor arguments, using the correct **mainnet** Blend pool address, not a testnet one. Populate this in `KNOWN_POOLS.mainnet` per the parameter-selection note above before this step, so you deploy against the address you intend to actually track.
3. Deploy mUSDC with that same reserved vault address and its constructor arguments.
4. Deploy the vault itself, **sourced by `ADMIN_KEY`** (the hardware-backed or multisig key from the prerequisites above, not `DEPLOYER`), using the same salt from step 1, with `admin`, `usdc`, `musdc`, and `adapter` as constructor arguments.
...
- **The admin key is compromised or suspected compromised.** `begin_migration`'s ~1-day timelock (see parameter selection above) is the intended detection-and-reaction window for a malicious `begin_migration` call specifically, and this is what the [alert keeper](./alert-keeper.md) exists to surface immediately, not something to notice by chance. If the key itself is compromised, not just a single malicious call observed, rotate via `transfer_admin`/`accept_admin` **immediately**, using a still-trusted signer if `ADMIN` is a multisig with a workable threshold remaining. A fully compromised single hardware key has no recovery path, which is exactly why "Prerequisites specific to mainnet" above pushes toward multisig rather than a single key, mainnet or not.
- **`set_paused` is called unexpectedly.** Withdrawals are deliberately left open even while paused (see `architecture/vault-contract.md`) so a pause can never trap funds. A pause alone, even a malicious one, does not put depositor funds at risk of loss, only of a temporarily blocked deposit path. Investigate via the alert keeper's event before assuming the worst.
- **Nothing above applies, but something still looks wrong.** `get_migration_snapshot()` and the admin-history endpoints (`getRpcAdminHistory`) are readable by anyone, not just the admin. Use them to independently verify on-chain state before acting, rather than trusting the frontend's cached view of it.
...
- [ ] `ADMIN` key custody finalized (hardware-backed or multisig, per "Prerequisites specific to mainnet") and its holder(s) briefed on the incident plan above
- [ ] `deploy-testnet.sh` adapted for mainnet, or the manual command sequence in "Deployment sequence" rehearsed at least once against testnet by whoever will run it for real
